...
const generateSubmitButton = () => $('section').append('<button>submit</button>');
...
const citiesAtoJ = (arr) => {
  let aj = [];
  arr.forEach(city => /^[A-J]/.test(city) ? aj.push(city): '');
  return aj;
};
...
const matchMonth = (input) => /oct|october/i.test(input);
