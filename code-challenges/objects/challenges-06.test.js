...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(obj => {
    if(obj.name === character){
      x = Object.values(obj)[2].length>0 ? true : false;
    }
  })
  return x;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(obj => {
    if(obj.name === character){
      x = Object.entries(obj)[2][1].length>0 ? true : false;
    }
  })
  return x;
};
...
const totalCharacters = (arr) => {
  let sizes = arr.map(obj => Object.values(obj).join().replace(/,,,/g, ',').split(',').length-1);
  let y =0;
  sizes.forEach(num => y = y+ num)
  return y;
}
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(obj =>{
    let x = Object.values(obj).join().replace(/,,,/g, ',').split(',').length-1
    let nobj = {house: obj.house, members: x};
    sizes.push(nobj);
  })
  return sizes;
};
